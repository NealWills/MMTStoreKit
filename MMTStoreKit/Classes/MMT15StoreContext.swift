...
@available(iOS 15.0, *)
public class MMT15StoreContext: ObservableObject, @unchecked Sendable {
    /// 更新
    ...
    // MARK: - 产品列表
    /// 产品列表 -  更新 ``MMT15StoreContext/productIds`` ID，通过 ``MMT15StoreContext/updateProducts(_:)`` 更新产品列表
    ...
    /// 购买交易，同时`更新`已购买的产品 ``MMT15StoreContext/purchasedProductIds`` ID
    ...
    /// ``MMT15StoreContext/init(productIds:)``
    public convenience init<Product: MMT15InAppProduct>(products: [Product]) {
...
@available(iOS 15.0, *)
@MainActor public extension MMT15StoreContext {
    // MARK: - 恢复购买
    /// 恢复购买
