//
//  MMT15StoreContext+Products.swift
//  StoreKitHelper
//
...
@available(iOS 15.0, *)
public extension MMT15StoreContext {
    /// 是否有购买
    /// 返回 `true` 需要购买
...
private extension VerificationResult where SignedType == Transaction {
    ...
    func verify() throws -> Transaction {
        switch self {
        case .unverified(let transaction, let error): throw MMT15StoreServiceError.invalidTransaction(transaction, error)
